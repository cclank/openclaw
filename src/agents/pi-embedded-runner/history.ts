...
export function getHistoryLimitFromSessionKey(
  ...
  if (kind === "dm" || kind === "direct") {
    if (providerConfig && userId && providerConfig.dms?.[userId]?.historyLimit !== undefined) {
  ...
  if (kind === "channel" || kind === "group") {
    // Check local optimization first
    // @ts-ignore
    const groupLimit = config.agents?.defaults?.groupHistoryLimit;
    return groupLimit ?? providerConfig?.historyLimit;
  }
